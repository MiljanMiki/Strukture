...
//**** NIJE BITNO STO JE TEZINSKI GRAF,SAMO NADJI PUT!
// very poopy refine this shits
bool GraphAsListsInt::pathWithoutEdge(int dataFirst, int dataLast, int dataEgdeSource, int dataEdgeDest)
{
	LinkedNodeInt* pocetniCvor = findNode(dataFirst);
	LinkedNodeInt* krajnjiCvor = findNode(dataLast);
	LinkedEdgeInt* zabranjenaGrana = findEdge(dataEgdeSource, dataEdgeDest);
	if (pocetniCvor == nullptr || krajnjiCvor == nullptr || zabranjenaGrana == nullptr)
		return false;
	if (pocetniCvor == krajnjiCvor)
		return true;

	LinkedNodeInt** nizCvorova = new LinkedNodeInt*[nodeNum]();//da bih posle odstampao samo taj put
	int pokazivacNaKraj = 0;
	nizCvorova[pokazivacNaKraj++] = pocetniCvor;
	bool nadjenPut = false;

	//da probam depthFirst search?
	StackAsArrayLinkedNodeInt stack(nodeNum);
	setStatusForAllNodes(1);
	LinkedNodeInt* ptr = pocetniCvor;
	stack.push(ptr);
	ptr->status = 2;

	while (!stack.isEmpty() && !nadjenPut)
	{
		ptr=stack.pop();
		ptr->status = 3;
		LinkedEdgeInt* pEdge = ptr->adj;
		...
		{
			if (pEdge != zabranjenaGrana && pEdge->dest == krajnjiCvor)
			{
				nizCvorova[pokazivacNaKraj++] = krajnjiCvor;
				nadjenPut = true;
			}
			else if (pEdge != zabranjenaGrana && pEdge->dest->status == 1)
			{
				nizCvorova[pokazivacNaKraj++] = pEdge->dest;
				stack.push(pEdge->dest);
				pEdge->dest->status = 2;
			}
			else if (pEdge == zabranjenaGrana)//veoma nije dobro resenje... ALII radi (barem za moj primer)
				pokazivacNaKraj--;
			pEdge = pEdge->link;
		}
	}

	if (nadjenPut && pokazivacNaKraj > 0)
	{
		for (int i = 0; i < pokazivacNaKraj; i++)
			if (nizCvorova[i] != nullptr)
				nizCvorova[i]->visit();
			else
				cout << "****GRESKA!!!! U NIZU CVOROVA SE NALAZI NULLPTR********";
		return true;
	}
	else
		return false;
