...
int GraphAsListsInt::brojCvorova(int data)//NE ukljucujuci taj prvi cvor
{
	LinkedNodeInt* ptr = findNode(data);
	if (ptr == nullptr)
		return 0;

	int brojObidjenihCvorova = 0;
	int* pokazivac = &brojObidjenihCvorova;
	setStatusForAllNodes(1);

	StackAsArrayLinkedNodeInt* stack=new StackAsArrayLinkedNodeInt(nodeNum);
	stack->push(ptr);
	ptr->status = 2;
	brojCvorova(stack,pokazivac);

	return brojObidjenihCvorova;
	
}

void GraphAsListsInt::brojCvorova(StackAsArrayLinkedNodeInt* stack,int* brojC)
{
	if (!stack->isEmpty())
	{
