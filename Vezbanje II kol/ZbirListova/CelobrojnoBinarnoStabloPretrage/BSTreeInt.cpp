...
/*	Odrediti zbir vrednosti svih listova zadatog 
	binarnog stabla.
	Izračunavanje obaviti korišćenjem rekurzivnog
	obilaska stabla.
*/
int BSTreeInt::modifikovanRekurzivanInorder(int* zbir,const BSTNodeInt* ptr)//nema veze sa inorder ali nema veze
{
	if (ptr != nullptr)
	{
		...
		modifikovanRekurzivanInorder(zbir, ptr->left);
		modifikovanRekurzivanInorder(zbir, ptr->right);

	}
	return *zbir;//useless,samo me mrzelo da menjam deklaraciju f-je
}

int BSTreeInt::zbirListova()//bilo koji obilazak,tipa inorder?
{
	int zbir = 0;
	int* p=&zbir;

	BSTNodeInt* ptr = root;
	//zbir=modifikovanRekurzivanInorder(zbir, ptr);
	//return zbir;

	modifikovanRekurzivanInorder(p, ptr);
	return zbir;

	//return modifikovanRekurzivanInorder(0, ptr);
}

int BSTreeInt::SumLeafs(BSTNodeInt* p)// ILI JE MOGLO SAMO OVO DA SE URADI...
{
	if (p != nullptr)
	{
		if (p->left == nullptr && p->right == nullptr)
			...
			return SumLeafs(p->left) + SumLeafs(p->right);
	}
	else
		return 0;
}
