...
int BSTreeInt::countSmaller(int d)
{
	...
	return countPodstabla(root, d, &count);

}

int BSTreeInt::countPodstabla(BSTNodeInt* start, int d, int* count)
{
	if (start != nullptr)
	{
		//if (start != root && start->getKey() > numOfElements - d && start->getKey() < numOfElements)
		//{
		//	*count += 1;
		//	//return *count;
		//}
		...
		/*if (start->right != nullptr)
			countPodstabla(start->right, d, &lokalniCount);
		if (start->left != nullptr)
			countPodstabla(start->left, d, &lokalniCount);*/

		if(start->left!=nullptr)
			obidjiPodstablo(start->left, d, &lokalniCount);
		if (start->right != nullptr)
			obidjiPodstablo(start->right, d, &lokalniCount);
		...
		if (start->right != nullptr)
			countPodstabla(start->right, d, count);
		if (start->left != nullptr)
			countPodstabla(start->left, d, count);
	}
	return *count;
}

//da li postoji nacin da se ne doda ova dodatna f-ja, obidjiPodstablo, vec da se ova
//logika nekako implementira u u gornju f-ju countPodStabla. Nzm da li bi prihvatili na kol
//jos jednu dodatnu rekurzivnu f-ju (zbog performansa)
void BSTreeInt::obidjiPodstablo(BSTNodeInt* start, int d, int* count)
{
	if (start != nullptr)
	{
		if (start->getKey() > numOfElements - d && start->getKey() < numOfElements)
		{
			*count+=1;
			return;//da ne bi nastavljalo dalje da trazi,dovoljno je samo jedan cvor da bude u opsegu
		}
		obidjiPodstablo(start->right, d, count);
		obidjiPodstablo(start->left, d, count);
	}
