...
void BSTreeInt::insert(int el,double val)
{
	...
	if (root == nullptr)    // stablo je prazno
		root = new BSTNodeInt(el,val);
	else if (par->isLT(el))
		par->right = new BSTNodeInt(el,val);
	else
		par->left = new BSTNodeInt(el,val);
	numOfElements++;
}
...
//najretardiranija konstrukcija zadatka IKADA
int BSTreeInt::brojCvorova(int keyMin, double valueMin)
{
	if (numOfElements == 0 || root == nullptr)
		return 0;

	int count = 0;
	brojCvorova(root, keyMin, valueMin, &count);
	return count;
}

//veci fokus se baca na desno podstablo zbog samih zahteva zadataka
//logicno da ce keyMin da bude negde u desnom podstablu,ako vec nije u root,
//a ako je root>keyMin,onda se broje oba podstabla
void BSTreeInt::brojCvorova(BSTNodeInt* start, int keyMin, double valueMin, int* count)
{
	if (start->getKey() > keyMin)
	{
		if (start->getValue() > valueMin)
		{
			if (start->right != nullptr)
				brojPodstablo(start->right, count);
			if (start->left != nullptr)
				brojPodstablo(start->left, count);
		}
		else
		{
			if (start->left != nullptr)
				brojCvorova(start->left, keyMin, valueMin, count);
			if (start->right != nullptr)
				brojCvorova(start->right, keyMin, valueMin, count);
		}
	}
	else
	{
...
void BSTreeInt::brojPodstablo(BSTNodeInt* start, int* count)
{
	*count += 1;
	if (start->right != nullptr)
		brojPodstablo(start->right, count);
