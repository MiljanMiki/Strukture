...
//u osnovi zar ne treba samo za koren da proverim?
//ako je barem jedan cvor neperfektan,onda je i celo podstablo kojem on pripada isto neperfektno
//umesto da idem od cvora do cvora i da brojim njegove "podcvorove", dovoljno je samo da proverim za koren??
bool BSTreeInt::isPerfect()
{
	...
	int brojCvorovaDesnog = 0, brojCvorovaLevog = 0;
	if(start->right!=nullptr)
		brojCvorove(start->right, &brojCvorovaDesnog);
	if (start->left != nullptr)
		brojCvorove(start->left, &brojCvorovaLevog);

	if (abs(brojCvorovaDesnog - brojCvorovaLevog) > 1)
		return false;
	return true;

}

void BSTreeInt::brojCvorove(BSTNodeInt* start, int* count)
{
	*count += 1;
	if (start->right != nullptr)
		brojCvorove(start->right, count);
	if (start->left != nullptr)
		brojCvorove(start->left, count);
}