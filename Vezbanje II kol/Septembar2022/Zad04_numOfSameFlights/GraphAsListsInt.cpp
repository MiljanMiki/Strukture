...
//predpostavljam da je ovo neusmeren graf?Letovi mogu biti dvosmerni.
int GraphAsListsInt::numOfSameFlights(int a1,int a2,int aEnd)
{
	LinkedNodeInt* airport1 = findNode(a1);
	LinkedNodeInt* airport2 = findNode(a2);
	LinkedNodeInt* airportEnd = findNode(aEnd);

	if (airport1 == nullptr || airport2 == nullptr || airportEnd == nullptr )
		return -1;
	...
	stack.push(airport1);
	LinkedNodeInt* previous = nullptr;
	bool putNadjen1 = false;

	airport1->status = 2;
	while (!stack.isEmpty() && !putNadjen1)
	{
		...
		while (pEdge != nullptr)
		{
			 if (pEdge->dest == airportEnd)//predpodstavka da su svi aerodromi razliciti
			{
				airportEnd->prev = pom;
				putNadjen1 = true;
				break;
	...
	if (putNadjen1)
	{
		...
		LinkedNodeInt* ptr = airportEnd;
		while (ptr != nullptr) //cisto da odstampam da znam koja putanja je nadjena
		{
			ptr->visit();
			ptr = ptr->prev;
		}
		...
		int count2 = 0;
		bool putNadjen2 = false;
		setStatusForAllNodes(1);

		stack.push(airport2);
		airport2->status = 2;
		while (!stack.isEmpty() && !putNadjen2)
		{
			LinkedNodeInt* pom = stack.pop(); pom->status = 2;
			pom->visit(); //stampa kroz koje cvorove prolazi da znam da li tacno izbacuje (moze da stampa i one cvorove kroz koje put realno ne prolazi)
			if (pom->prev != nullptr || pom==airport1)
				count2++;

			LinkedEdgeInt* pEdge = pom->adj;
			while (pEdge != nullptr)
			{
				if (pEdge->dest == airportEnd)
				{
					putNadjen2 = true;
					break;
				}
