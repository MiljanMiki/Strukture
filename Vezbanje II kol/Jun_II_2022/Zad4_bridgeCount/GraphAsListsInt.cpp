...
void GraphAsListsInt::toUndirected()
{
	...
	while (ptr != nullptr)
	{
		...
		while (pEdge != nullptr)
		{
			LinkedEdgeInt* suprotnaGrana = findEdge(pEdge->dest->node, ptr->node);
			if (suprotnaGrana == nullptr)
...
//zadak se svodi na to da se nadje svaki moguci put u grafu,pa da se zatim vrati onaj najkraci
//po konstrukciji zadatka logicno je da je graf neorijentisan
int GraphAsListsInt::bridgeCount(int a, int b)
{
	LinkedNodeInt* first = findNode(a);
	LinkedNodeInt* last = findNode(b);
	if (start == nullptr || nodeNum == 0 || first == nullptr || last == nullptr)
		return -1;

	int minCount = nodeNum + 1;//najduzi moguci put u grafu (treba bez +1...),meni treba najmanji izmedju a i b
	...
	while (pEdge != nullptr)
	{
		...
		rekurzivanDFS(pEdge->dest, last,&count,&minCount);
		//if (count < minCount)//nepotrebno?
		//	minCount = count;
...
void GraphAsListsInt::rekurzivanDFS(LinkedNodeInt* start, LinkedNodeInt* finish,int* count,int* minCount)
{
	*count += 1;
	start->status = 2;
	...
	while (pEdge != nullptr)
	{
		int trenutniBrojObidjenihGrana = *count;//koliko je trebalo grana da se dodje do trenutnog cvora
		if (pEdge->dest == finish)				
		{
			*count += 1;
			if (*count < *minCount)
				*minCount = *count;
			start->status = 1;//da bi se ovaj cvor ponovo preradio u nekoj drugoj rekurziji
			return;
		}
		if (pEdge->dest->status == 1)
			rekurzivanDFS(pEdge->dest, finish, &trenutniBrojObidjenihGrana, minCount);

		pEdge = pEdge->link;
	}
	//*count -= 1;
	start->status = 1;// nakon sto se zavrsi prerada tekuceg cvora,mora da se vrati status na 1
					  //da bi mogao ponovo da se obidje u nekoj drugoj rekurziji
